...
make_release $VERSION $FORCE="":
   sed -i 's/^version = .*/version = "'$VERSION'"/g' velox-cli/Cargo.toml velox/Cargo.toml
   ...
   git add velox/Cargo.* velox-cli/Cargo.* Cargo.*
...
docker_release $TAG:
  #docker login -u aerol-ai ghcr.io
  #~/.depot/bin/depot build --project v4z5w7md33 --platform linux/arm/v7,linux/arm64,linux/amd64 -t ghcr.io/aerol-ai/velox:$TAG -t ghcr.io/aerol-ai/velox:latest --push .
  ...
  docker buildx build \
    ...
    --cache-from type=registry,ref=ghcr.io/aerol-ai/velox:cache \
    --cache-to type=registry,mode=max,image-manifest=true,oci-mediatypes=true,ref=ghcr.io/aerol-ai/velox:cache \
    -t ghcr.io/aerol-ai/velox:$TAG \
    -t ghcr.io/aerol-ai/velox:latest \
    --push .
